...
hickit -i $name.ca.female.contacts.pairs.gz -u -o - | bgzip > $name.ca.female.impute.pairs.gz

mkdir 3D.$name.ca.female
cd 3D.$name.ca.female

# generate 3D structures (with 3 replicates)
for rep in `seq 1 5`
do
  hickit -s${rep} -M -i ../$name.ca.female.impute.pairs.gz -Sr1m -c1 -r10m -c2 -b4m -b1m -O $name.1m.${rep}.3dg -b200k -O $name.200k.${rep}.3dg -D5 -b50k -O $name.50k.${rep}.3dg -D5 -b20k -O $name.20k.${rep}.3dg
done

#######################
# convert from hickit to dip-c formats, and remove repetitive regions from 3D structures
../dip-c-master/scripts/hickit_pairs_to_con.sh ../$name.ca.female.contacts.pairs.gz
../dip-c-master/scripts/hickit_impute_pairs_to_con.sh ../$name.ca.female.impute.pairs.gz

for rep in `seq 1 5`
do
  ../dip-c-master/scripts/hickit_3dg_to_3dg_rescale_unit.sh $name.20k.${rep}.3dg
  ../dip-c-master/dip-c clean3 -c ../$name.ca.female.impute.con.gz $name.20k.${rep}.dip-c.3dg > $name.20k.${rep}.clean.3dg # remove repetitive (contact-less) regions
done
